...
!function () {
    ...
    var color_random = d3.scale.category20();
	...
	function drawPart(data, id, p){
		...
		var mainbar = d3.select("#"+id).select(".part"+p).select(".mainbars")
            ...
            .attr("xlink:href", function(d,i){ 
                if(data.keys[p][i].indexOf("Wikipedia") > -1)
                    //console.log(data.keys[p][i]);
                    return "https://en.wikipedia.org/wiki/"+data.keys[p][i].slice(0,data.keys[p][i].indexOf(" "));
            })
            .attr("target","_blank")
            //danail
            .append("g")
		...
		mainbar.append("text").attr("class","barlabel")
			...
			.text(function(d,i){ 
			     if(data.keys[p][i].indexOf("Wikipedia") > -1)
                    return data.keys[p][i].slice(data.keys[p][i].indexOf("-")+ 2);
                 else
            ...
            .text(function(d,i) { 
                if(data.keys[p][i].indexOf("Wikipedia") > -1)
                    return ""+data.keys[p][i].slice(0,data.keys[p][i].indexOf(" "));
            });
	...
	function drawHeader(header, id){
		...
		[0,1].forEach(function(d){
			...
			h.append("line").attr("x1",c4[d]-10).attr("y1", -2)
				.attr("x2",c2[d]+10).attr("y2", -2).style("stroke","black")
	...
	function transitionPart(data, id, p){
	   ...
	   //added by danial
	   mainbar.attr( "fill-opacity", function(d){
                if( d.percent> 0.01 )
                    return 1;
                else
                    return 0.2;
	...
	function transitionEdges(data, id){
		...
		d3.select("#"+id).select(".edges").selectAll(".edge").data(data.edges)
			...
			.style("opacity",function(d){ return (d.h1 ==0 || d.h2 == 0 ? 0 : 0.3);});	
	}
	...
	bP.selectSegment = function(data, m, s){
		data.forEach(function(k){
            ...
            //added by danial
            d3.select("#"+k.id).select(".part"+m).select(".mainbars")
				.selectAll(".mainbar").attr( "fill-opacity", 0.3 ); // change to .3
	...
	bP.deSelectSegment = function(data, m, s){
		data.forEach(function(k){
            ...
            //added by danial
            d3.select("#"+k.id).select(".part"+m).select(".mainbars")
				.selectAll(".mainbar").attr( "fill-opacity", 1 );
