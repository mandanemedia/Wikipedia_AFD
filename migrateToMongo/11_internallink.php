...
$sql = "select * from comment_internallink;";
$records = array();
if ( $result=mysqli_query($conn,$sql) or die(mysqli_error($conn)) )
    while ( $obj=mysqli_fetch_object($result) )
    {
        $obj->ILinkLabel = utf8_encode($obj->ILinkLabel);
        $obj->ILinkURL = utf8_encode($obj->ILinkURL);
        $records[] = $obj;
    }
echo "<pre>";
//print_r($records);
...
foreach ($records as $record)
{
    ...
    $afds->update(
         ...
         array('$push' => array("comment.$.internallink"=>$record) )
    );
}
