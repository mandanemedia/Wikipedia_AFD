...
$sql = "select * from comment_externallink;";
$records = array();
if ( $result=mysqli_query($conn,$sql) or die(mysqli_error($conn)) )
    while ( $obj=mysqli_fetch_object($result) )
    {
        $obj->ELinkLabel = utf8_encode($obj->ELinkLabel);
        $obj->ELinkURL = utf8_encode($obj->ELinkURL);
        $records[] = $obj;
    }
echo "<pre>";
//print_r($records);
...
foreach ($records as $record)
{
    ...
    $afds->update(
         ...
         array('$push' => array("comment.$.externallink"=>$record) )
    );
}
